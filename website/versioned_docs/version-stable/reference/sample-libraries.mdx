...
At a technical level, _Floe sample libraries_ are a new, custom format; they're open, portable folders of audio files and configuration files developed with Floe's [developer tools](../develop/develop-libraries). Typically, a sample library is in a folder with a name like `Developer Name - Library Name`.
