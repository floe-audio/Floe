...
## Current status
{data["latest-beta-release-version"] ? (<>There is a Floe beta version live: {data["latest-beta-release-version"].version}. This is newer than the latest stable release, {data["latest-stable-release-version"]}.</>) : (<>There are no beta versions available. The latest stable release, {data["latest-stable-release-version"]}, is the newest possible version of Floe.</>)}
