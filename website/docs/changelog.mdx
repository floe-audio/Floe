...
# Changelog
## 2.0.4-beta.3
- Show a parameter's macro-adjusted value in its value popup, along with which macros are changing it
- Fix wrapped text such as preset descriptions sometimes starting a line with a full stop, for example when the text contained an ellipsis
- Fix the per-note markers on a layer's Volume slider ignoring macros, so they now sit at the level you actually hear

## 2.0.4-beta.2
### Browsers
The preset, Instrument and IR browsers have been redesigned. See the [browsers docs](usage/browsers) for a full walkthrough.

#### What's different
- Add Browse mode, now the default: click through a few short pages to narrow down to a library, preset bank, folder or tag, rather than scanning a long tree of filters. Switch to Filter mode <FAIcon icon="fa-solid fa-filter" /> for the previous behaviour
...
- Make Favourites <FAIcon icon="fa-solid fa-star" /> narrow the current list rather than act as a separate filter, so you can see just your favourites within a preset bank, for example
...
- Make <FAIcon icon="fa-solid fa-location-arrow" /> also open the current item's library or preset bank in the right panel in Browse mode, rather than just scrolling to the item
...
- Fix clicking an already-loaded item unloading it. This was easy to do by accident and left the preset blank. To unload, use the <FAIcon icon="fa-solid fa-xmark" /> on the selector instead

### Other changes
- Improve performance of the audio engine to be easier on the CPU, up to 2.5x faster in heavy granular use cases
- Add a seed option to granular layers and random LFO shapes <FAIcon icon="fa-solid fa-seedling" />, so a preset can save the exact grain scatter or LFO movement. Choose Fixed for the same pattern on every note, or Fixed Per Key for a repeatable variation on each key
