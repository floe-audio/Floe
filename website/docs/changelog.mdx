...
- Add MPE (MIDI Polyphonic Expression) support: enable it in the Instance Config panel for per-note pitch bend, press (channel pressure) and slide (CC74), and choose what press and slide control on each layer's Config tab. CLAP per-note expressions (pitch, pressure, brightness) are also supported
