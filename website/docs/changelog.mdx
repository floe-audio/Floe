...
- Add a seed option to granular layers <FAIcon icon="fa-solid fa-seedling" />, so a preset can save the exact grain scatter. Choose Fixed for the same grains on every note, or Fixed Per Key for a repeatable variation on each key
