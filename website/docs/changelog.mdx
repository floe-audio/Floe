...
## 2.0.4-beta.2
### Browsers
The preset, Instrument and impulse response browsers have been redesigned around getting to a sound with less fuss:
- Add a Browse mode, now the default. Rather than a tree of filters, you start from a short list of places to look - your libraries or preset banks, tags, authors - and drill down one page at a time until you're looking at just one library, folder or tag. Less scrolling, fewer things to consider, and it's much harder to lose your place. A trail at the top of the panel, with back and forward arrows, takes you back up
- Keep everything the browser could do before in a Filter mode: the full tree of libraries, folders and tags, where several filters can be combined with Match all (AND) or Match any (OR). Switch modes with the toggle at the bottom of the right panel; Floe remembers your choice
- Remember where you were in Browse mode, so a new instance of Floe opens the browser there
- Put the results on the left and the places to look on the right. The browser opens directly beneath the box you clicked, so the sounds are right under your cursor, like a menu
- When you click on an item, if you then move your cursor out of the browser window, it closes - a faster way to quickly select a new item
- Make the box that opened the browser part of it. The preset box, a layer's Instrument selector or the IR selector stays visible and usable while the browser is open, showing the current item and keeping its previous/next/random and unload buttons, so the browser no longer duplicates them and has no title bar of its own
- Add a line above the results saying what you're looking at, such as "Showing from Abyss preset bank" or "Showing favourites matching "pad" with Tag: Ambient". Each part is a button with an x that removes that filter, clears the search or steps back out of the library or preset bank
- Move search and Favourites into a toolbar at the bottom of the results, alongside a new jump-to-current button that scrolls to the loaded item, clearing anything that hides it first, and in Browse mode opens its library or preset bank in the right panel
- Make Favourites a refinement of whatever is currently shown rather than a filter of its own: open a preset bank and turn on Favourites to see just your favourites within it. Search works the same way, and when nothing matches within the current selection, a button offers to search everything instead
- Make the preset search match preset names only. It used to match preset bank and folder names too, which made results surprisingly broad
- Make the browser resizable: drag the bottom-right corner to change its size, or the border between the two panels to change their widths. Double-click either to reset
- Group preset banks into Factory preset banks and User presets when you have both kinds. Preset bank makers can mark a bank as factory with a new `factory = true` key in `floe-preset-bank.ini`; banks whose folder name contains "factory presets" count automatically
- Scroll to the currently loaded item when the browser opens, and show a message in the results when nothing matches
- Rename the preset browser's Libraries filter to Libraries used, since it lists the libraries a preset draws its sounds from rather than preset banks
- Improve spacing, headings and layout throughout the browsers
...
- Add a display to the Distortion and Bitcrush effects showing a preview of the current shape
...
- Fix a common issue where clicking an already-loaded item in the Instrument, IR or preset browser unloaded it. It was easy to trigger by accident and left the preset blank. Instead, use the x or right-click menu on the Instrument or IR selector, or the top panel's reset.
...
- Fix voice marker persisting in Granular Playback mode even when the playback has finished
