...
# Changelog
## 2.0.4-beta.2
- Add a Simple view to the browsers, now the default: libraries and preset banks are a plain list, and clicking one shows only its folders. The full filter tree with AND/OR modes is available from the same menu as Advanced view
- Scroll the browser to the currently loaded item when it opens
- Show a notice in the browser when the current item is hidden by the filters or search, with a Show button that clears them, or when it isn't listed at all, such as when its library isn't installed
- Disable the browser's scroll-to-current button when there's nothing to scroll to, with a tooltip saying why
- Show a message in the browser's results list when nothing matches the filters or search
- Improve performance of the audio engine, up to 2.5x faster in heavy granular use cases
- Add 'Swap with Left Layer' and 'Swap with Right Layer' to the layer's right-click menu, moving the whole layer including its macro assignments
- Add Swap with Left and Swap with Right to a macro's right-click menu, for rearranging the macro order without changing what any of them do
- Add a display to the distortion effect showing a sine wave shaped by the current settings
...
- Add a gain reduction meter to the compressor effect, showing how much it's turning the signal down
- Add a graph showing the High-pass filter's response, draggable to set the cutoff
- Hide tooltips when the mouse leaves Floe's window rather than leaving the last hovered control's tooltip showing
- Hide the play mode, reverse and loop mode controls for waveform Instruments, since a waveform ignores them, and say so in the caption below the waveform
- Show the voices of a waveform Instrument on its waveform display, placed left to right by pitch
- Fix the Ring Mod (44.1k) legacy distortion type modulating 4 times too fast
- Fix granular notes never ending when the playhead reaches the end of a sample that has no loop, leaving a stuck marker on the waveform and using up voices
- Fix voice marker persisting in Granular Playback mode even when the playback has finished
- Clicking an already-loaded item in the Instrument, IR or preset browser no longer unloads it, which was easy to trigger by accident and left the preset blank. Use the browser's Unload button or the top panel's reset instead
