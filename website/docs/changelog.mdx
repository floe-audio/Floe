...
# Changelog
## 2.0.4-beta.1
- Redesign the effects switchboard cards: each card now has a checkbox — tick it to add the effect to the rack, untick it to remove it. Clicking a card that's already in the rack scrolls and flashes to its place in the chain.
...
- Add right-click on the effects switchboard to 'Move Unused FX to Bottom', which groups all the turned-on effects at the top without changing their order
