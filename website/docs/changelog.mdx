...
# Changelog
## 2.0.0-beta.5
- Preset bank `floe-preset-bank.ini` supports an optional `icon_library` field to pick which library's icon represents the bank
- Hold shift while dragging waveform handles (loop, crossfade, offset) for finer adjustment
- Show 2 decimal places for layer loop start/end, crossfade and sample offset values
