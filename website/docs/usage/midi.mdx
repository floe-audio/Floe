...
Choose what press and slide control per layer on the [Config tab](./layers#config-tab). You can set a target (volume, filter cutoff, timbre, or LFO amount) and an amount (which can be negative to invert the response, and even have values beyond 100% to reach the full effect with a smaller gesture).
