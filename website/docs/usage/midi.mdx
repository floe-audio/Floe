...
Enable MPE from the Instance Config panel — open it from the top-panel menu (the three-dots button). The setting is saved with your DAW project and stays as-is when you load presets.
...
The smoothing setting in the Instance Config panel controls how gradually press and slide changes are applied: lower values are more responsive, higher values are smoother.
