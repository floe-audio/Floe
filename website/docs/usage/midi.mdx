...
MIDI CC mappings are saved with your DAW project, and apply only to the instance you created them on — a new instance of Floe starts without them, unless you've made them the default (see [Performance Controls: Settings for new instances](./performance-controls#settings-for-new-instances)).
...
Choose what press and slide control per layer on the [Config tab](./layers#config-tab): a target (volume, filter cutoff, or timbre) and an amount, which can be negative to invert the response, or beyond 100% to reach the full effect with a smaller gesture. For the volume target, press restores volume up to the layer's slider level, and slide acts as a per-note fader — the top of its travel is the slider level. For the filter cutoff and timbre targets, press adds an offset and slide is bipolar with its centre position neutral. The timbre target offsets the master timbre control per note.
