...
Configure key ranges on each [layer's **Play tab**](./layers#play-tab) using the low and high key controls. Click and drag these handles to set the range that layer will respond to.
