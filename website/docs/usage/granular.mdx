...
- Pairs naturally with the [Arpeggiator](./arpeggiator), [LFOs](./layers) and [Macros](./macros) for evolving, performance-ready textures.
...
- Time-stretching a sample to fit a slower tempo without pitching it down.
- Slow, drifting playback of long evolving samples.
- Playing the sample backwards with the **Reverse** toggle — grains keep their own size and pitch, but the playhead sweeps the sample from end to start.
...
- The Position parameter is automatable — pair it with an LFO or a [macro](./macros) for movement that doesn't require you to ride the knob by hand.
- Granular Fixed mode is monophonic in feel but polyphonic in voicing: each note you play spawns its own independent grain cloud at the chosen position.
