...
### Seed
Click the <FAIcon icon="fa-solid fa-seedling" /> seedling next to the play mode menu to choose where the grain randomness comes from. The icon lights up when the layer uses its own seed.

- **Random**: the default. Every note scatters its grains in a fresh way, which suits most sounds, especially dense textures. To make a whole performance repeat exactly in your DAW, use the [Reproducibility](./reproducibility) settings in Performance Controls.
