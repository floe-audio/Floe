...
Floe has elements that vary between performances: round robin sample selection cycles through different samples for repeated notes, and the granular engine uses randomness for grain parameters such as pan, detune, and spread. By default, these are not reset between plays, meaning each performance is slightly different.
