...
Reproducibility settings live in the [Performance Controls](./performance-controls) panel.
...
- The granular engine's randomised grain properties, unless a layer uses its own [granular seed](./granular#seed)
- The LFO's random waveforms (Random Steps and Random Glide), unless a layer uses its own [LFO seed](./layers#lfo-tab)
...
- Like the rest of the [Performance Controls](./performance-controls) panel, these settings belong to this specific instance and loading a preset leaves them untouched — so you can audition presets without losing your reproducibility setup.
