...
Floe can install sample libraries and presets from package files (`.floe-pkg` or `.zip`).
...
Download your chosen package(s) (`.floe-pkg` or `.zip` files containing sample libraries and presets).
...
Instead of using the 'install package' button, you can manually install libraries and presets by extracting the package file into the correct folders. Packages are just ZIP files. If you have a `.floe-pkg` file, rename it to `.zip` first and then extract it using any ZIP extraction tool - often just a matter of double-clicking on it.
...
No conversion needed! Just change the file extension. There are countless ways to do this. As quick guidance for our users:
