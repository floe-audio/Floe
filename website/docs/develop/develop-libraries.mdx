...
We made these tools for our own use and are sharing them with the community. Consider contributing or [sponsorship](../about-the-project/sponsorship) if you'd like to see further investment in this area.
...
This file uses [Floe's Lua functions](./library-lua-scripts) to create the library, create instruments, and add regions and impulse responses. It can reference any file in the library using relative paths.
...
1. Create a new folder in one of Floe's [sample library folders](../usage/folders). We recommend naming it 'Author Name - Library Name'.
...
1. Use the `floe.new_library()` function to create your library, filling in all the fields marked `[required]` in the [Floe's Lua reference](./library-lua-scripts).
...
1. Floe automatically detects whenever any library file changes and will tell you if there's any errors. If a library is correctly configured, it will instantly appear in Floe. For a fuller view of what was parsed — regions, loop points, missing samples — run [`floe-library-inspector`](#library-inspector).
...
{data["latest-beta-release-version"]?.["Floe-Library-Inspector-Windows"] ? (
<ul>
  <li><FAIcon icon="fa-brands fa-windows" /> <a href={data["latest-beta-release-version"]["Floe-Library-Inspector-Windows"].url}>{data["latest-beta-release-version"]["Floe-Library-Inspector-Windows"].name}</a> ({data["latest-beta-release-version"]["Floe-Library-Inspector-Windows"].size})</li>
  <li><FAIcon icon="fa-brands fa-apple" /> <a href={data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Apple-Silicon"].url}>{data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Apple-Silicon"].name}</a> ({data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Apple-Silicon"].size})</li>
  <li><FAIcon icon="fa-brands fa-apple" /> <a href={data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Intel"].url}>{data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Intel"].name}</a> ({data["latest-beta-release-version"]["Floe-Library-Inspector-macOS-Intel"].size})</li>
  <li><FAIcon icon="fa-brands fa-linux" /> <a href={data["latest-beta-release-version"]["Floe-Library-Inspector-Linux"].url}>{data["latest-beta-release-version"]["Floe-Library-Inspector-Linux"].name}</a> ({data["latest-beta-release-version"]["Floe-Library-Inspector-Linux"].size})</li>
</ul>
) : (
<p><em>Downloads will appear here with the next beta release.</em></p>
)}

<CodeBlock>
{data["library-inspector-help"]}
</CodeBlock>
...
Floe allows libraries to have a custom background image that appear behind the interface when the library is loaded. These images are configured in your [Lua script](./library-lua-scripts) through Floe's `new_library` function.
