...
| Layer 1/Playback/Granular | Length | 217 | Duration of each grain snippet |
| Layer 1/Playback/Granular | Speed | 218 | How fast the grain position moves through the sample |
| Layer 1/Playback/Granular | Position | 219 | Where in the sample grains are sourced from |
| Layer 1/Playback/Granular | Density | 220 | Controls how densely grains overlap, relative to the grain length. At the midpoint, grains play end-to-end. Lower values add gaps between grains for a sparse texture; higher values make grains overlap for a denser, richer sound |
| Layer 1/Playback/Granular | Spread | 221 | Region around the playhead where grains can start from. Small values focus grains near the playhead, large values spread them across a wider area |
| Layer 1/Playback/Granular | Smooth | 222 | Crossfade between grains to remove clicks. Low is hard cuts, high is full overlap fade |
| Layer 1/Playback/Granular | Pan | 223 | Randomise the stereo position of each grain. At 0% all grains play centred, at 100% grains can be panned anywhere from fully left to fully right |
| Layer 1/Playback/Granular | Detune | 224 | Randomise the pitch of each grain. At 0% all grains play at the original pitch, at 100% grains can be detuned up to a semitone up or down |
| Layer 1/Playback/Granular | Direction | 225 | Chance that grains spawn playing in the opposite direction to the main playhead. At 0% all grains play in the main direction, at 100% there's a 50/50 chance of each grain playing forwards or backwards |
| Layer 1/Playback/Granular | Harmony | 226 | Chance that grains spawn at one of the selected harmony intervals instead of the root pitch. Configure which intervals are active using the Intervals button |
...
| Layer 1/Arp | Auto Rate | 236 | Let Floe decide the rate so that the loop plays near to its original recorded rate rather than having to add large silent gaps between slices. The rate that Floe chooses varies depending on your host tempo. |
| Layer 1/Arp | Polyrate | 237 | Each octave plays at a different rate. Double means each octave up is 2x faster. 3:2 and 4:3 create polyrhythmic relationships between octaves |
...
| Layer 2/Playback/Granular | Length | 377 | Duration of each grain snippet |
| Layer 2/Playback/Granular | Speed | 378 | How fast the grain position moves through the sample |
| Layer 2/Playback/Granular | Position | 379 | Where in the sample grains are sourced from |
| Layer 2/Playback/Granular | Density | 380 | Controls how densely grains overlap, relative to the grain length. At the midpoint, grains play end-to-end. Lower values add gaps between grains for a sparse texture; higher values make grains overlap for a denser, richer sound |
| Layer 2/Playback/Granular | Spread | 381 | Region around the playhead where grains can start from. Small values focus grains near the playhead, large values spread them across a wider area |
| Layer 2/Playback/Granular | Smooth | 382 | Crossfade between grains to remove clicks. Low is hard cuts, high is full overlap fade |
| Layer 2/Playback/Granular | Pan | 383 | Randomise the stereo position of each grain. At 0% all grains play centred, at 100% grains can be panned anywhere from fully left to fully right |
| Layer 2/Playback/Granular | Detune | 384 | Randomise the pitch of each grain. At 0% all grains play at the original pitch, at 100% grains can be detuned up to a semitone up or down |
| Layer 2/Playback/Granular | Direction | 385 | Chance that grains spawn playing in the opposite direction to the main playhead. At 0% all grains play in the main direction, at 100% there's a 50/50 chance of each grain playing forwards or backwards |
| Layer 2/Playback/Granular | Harmony | 386 | Chance that grains spawn at one of the selected harmony intervals instead of the root pitch. Configure which intervals are active using the Intervals button |
...
| Layer 2/Arp | Auto Rate | 396 | Let Floe decide the rate so that the loop plays near to its original recorded rate rather than having to add large silent gaps between slices. The rate that Floe chooses varies depending on your host tempo. |
| Layer 2/Arp | Polyrate | 397 | Each octave plays at a different rate. Double means each octave up is 2x faster. 3:2 and 4:3 create polyrhythmic relationships between octaves |
...
| Layer 3/Playback/Granular | Length | 537 | Duration of each grain snippet |
| Layer 3/Playback/Granular | Speed | 538 | How fast the grain position moves through the sample |
| Layer 3/Playback/Granular | Position | 539 | Where in the sample grains are sourced from |
| Layer 3/Playback/Granular | Density | 540 | Controls how densely grains overlap, relative to the grain length. At the midpoint, grains play end-to-end. Lower values add gaps between grains for a sparse texture; higher values make grains overlap for a denser, richer sound |
| Layer 3/Playback/Granular | Spread | 541 | Region around the playhead where grains can start from. Small values focus grains near the playhead, large values spread them across a wider area |
| Layer 3/Playback/Granular | Smooth | 542 | Crossfade between grains to remove clicks. Low is hard cuts, high is full overlap fade |
| Layer 3/Playback/Granular | Pan | 543 | Randomise the stereo position of each grain. At 0% all grains play centred, at 100% grains can be panned anywhere from fully left to fully right |
| Layer 3/Playback/Granular | Detune | 544 | Randomise the pitch of each grain. At 0% all grains play at the original pitch, at 100% grains can be detuned up to a semitone up or down |
| Layer 3/Playback/Granular | Direction | 545 | Chance that grains spawn playing in the opposite direction to the main playhead. At 0% all grains play in the main direction, at 100% there's a 50/50 chance of each grain playing forwards or backwards |
| Layer 3/Playback/Granular | Harmony | 546 | Chance that grains spawn at one of the selected harmony intervals instead of the root pitch. Configure which intervals are active using the Intervals button |
...
| Layer 3/Arp | Auto Rate | 556 | Let Floe decide the rate so that the loop plays near to its original recorded rate rather than having to add large silent gaps between slices. The rate that Floe chooses varies depending on your host tempo. |
| Layer 3/Arp | Polyrate | 557 | Each octave plays at a different rate. Double means each octave up is 2x faster. 3:2 and 4:3 create polyrhythmic relationships between octaves |
...
| Master | Legacy Velocity To Volume Strength | 1 | Legacy parameter. The amount that the MIDI velocity affects the volume of notes; 100% means notes will be silent when the velocity is very soft, and 0% means that notes will play full volume regardless of the velocity |
...
| Macro | 1 | 101 | A macro that can be assigned to any parameter in the plugin. The macro will affect all parameters that are assigned to it. |
| Macro | 2 | 102 | A macro that can be assigned to any parameter in the plugin. The macro will affect all parameters that are assigned to it. |
| Macro | 3 | 103 | A macro that can be assigned to any parameter in the plugin. The macro will affect all parameters that are assigned to it. |
| Macro | 4 | 104 | A macro that can be assigned to any parameter in the plugin. The macro will affect all parameters that are assigned to it. |
