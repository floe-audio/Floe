...
When installing has finished, Floe shows a completion dialogue with a couple of optional actions: sending the package file to the trash (it isn't needed after installation), and, for packages whose preset bank names a default preset, loading that preset. If Floe was blank at the time, that default preset is loaded for you automatically. Press _Done_ when you've finished with the dialogue. No restart required.
