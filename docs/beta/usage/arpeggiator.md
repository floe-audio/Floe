...
In _Played Notes_ mode your own playing is folded in as well: how hard you play the notes that trigger the sequence (typically MIDI velocity) scales each step's velocity rather than replacing it. At the softest playing (MIDI velocity 1) the step velocities are 50% as intense, while at the hardest (velocity 127) they reach their full intensity. The pattern's accents therefore survive intact while your performance shapes the dynamics over the top of them. In _Fixed Notes_ mode the steps stand alone and how hard you play is ignored.
