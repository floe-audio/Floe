...
Click the seedling next to the play mode menu to choose where the grain randomness comes from. The icon lights up when the layer uses its own seed.

-   **Random**: the default. Every note scatters its grains in a fresh way, which suits most sounds, especially dense textures. To make a whole performance repeat exactly in your DAW, use the [Reproducibility](/docs/beta/usage/reproducibility) settings in Performance Controls.
-   **Fixed**: every note plays exactly the same grains, chosen by **Seed**. Useful when Density is low and you can hear the individual grains. Hold a chord and all the notes follow the same movement, each at its own pitch.
-   **Fixed Per Key**: like Fixed, but each key gets its own pattern of grains. C3 always sounds the same, and D3 has a different pattern that also repeats every time you play it.

The seed is saved with the preset, so Fixed and Fixed Per Key let you save the exact grain scatter you heard while designing a sound. Different Seed values give different results.
