...
> Learn how to use macros in Floe to control multiple parameters with a single knob
...
_Complete workflow: creating macro destinations, adjusting amounts, and renaming_
...
![Macro connect GUI](/assets/images/macros-connect-0e2ed5e0eca68332962003ef358d1107.png)
...
![Macro destination amount](/assets/images/macros-destination-amount-f36f619e2450896b6a87dcd7f566263a.png)
...
Drag the knob to adjust the amount, or double-click to type an exact value. Right-clicking the knob opens a menu with further options, including entering a value, copying and pasting the amount between destinations, resetting it, and inverting it.

Hovering over the destination amount knob shows what the destination parameter will be when the macro is at 0% and at 100%, plus its current value, in the parameter's own units. For example, a volume destination might show "-60 dB to 0 dB (now -12.4 dB)": the parameter sits at -60 dB when the macro is at 0%, reaches 0 dB when the macro is at 100%, and is currently at -12.4 dB.
...
-   Not all 4 macros have to be used all the time. However, when not using 4, avoid put unused macros between or before active ones. Right-click a macro and choose Swap with Left or Swap with Right to move macros so that macros are filled from left to right with unused macros at the end.
