...
The default preset is what a new instance of Floe opens with, instead of the blank state. When your DAW loads a saved project, the project's own state takes over as usual.

There are two ways to set it. Right-click any preset in the [browser](/docs/beta/usage/browsers) and choose _Set as Default Preset_; the preset is marked with a icon in the browser afterwards. Or, if the preset you want is already loaded, open Preferences > General and press _Use Current Preset_ in the Default Preset row. The same row shows the current default and has a button to clear it, which returns Floe to opening on the blank state.

Preset banks can suggest a default preset too. Installing a package that contains such a bank applies the suggestion, so a product you've just installed can open on one of its own presets. The most recently installed bank's suggestion wins, and a suggestion never replaces a default preset that you set yourself. If Floe is blank when the install finishes, the suggested preset is loaded straight away; otherwise the install-complete dialogue offers to load it.
