...
Reproducibility settings live in the [Performance Controls](/docs/beta/usage/performance-controls) panel.
...
-   The granular engine's randomised grain properties, unless a layer uses its own [granular seed](/docs/beta/usage/granular#seed)
-   The LFO's random waveforms (Random Steps and Random Glide), unless a layer uses its own [LFO seed](/docs/beta/usage/layers#lfo-tab)
-   Other smaller sources of variation throughout the engine

Left alone, these would drift between plays. The Reproducibility tab is where you control them.
...
-   Like the rest of the [Performance Controls](/docs/beta/usage/performance-controls) panel, these settings belong to this specific instance and loading a preset leaves them untouched — so you can audition presets without losing your reproducibility setup.
