...
// Minor modifications made by Sam Windell
    ...
    for (self.modules.items) |*module| {
        if (address >= module.base_address and address < module.base_address + module.size) {
            ...
            if (coff_obj.strtabRequired()) {
                ...
                // NOTE (Sam): this is a fix I made compared to the std version - we now correctly handle UNC
                // paths.
                const prefixed_path = std.os.windows.wToPrefixedFileW(null, name_buffer[0..len :0]) catch return error.MissingDebugInfo;

                const coff_file = fs.openFileAbsoluteW(prefixed_path.span(), .{}) catch |err| switch (err) {
                    error.FileNotFound => return error.MissingDebugInfo,
                    else => return err,
                };
            ...
            errdefer {
                if (module.mapped_file) |mapped_file| mapped_file.deinit();
                // NOTE (Sam): we should make an issue for this upstream. Zig's std version doesn't do this nulling
                // of the mapped_file field - so there's a crash if deinit() is called twice, once here and once in
                // SelfInfo.deinit().
                module.mapped_file = null;
            }
...
pub const Module = switch (native_os) {
    ...
    .uefi, .windows => struct {
        ...
        pub fn getDwarfInfoForAddress(self: *@This(), allocator: Allocator, address: usize) !?*Dwarf {
            ...
            // NOTE(Sam): we have fixed this since the std version has a compile error
            if (self.dwarf != null) return &self.dwarf.?;
            return null;
        }
    },
...
fn readCoffDebugInfo(allocator: Allocator, coff_obj: *coff.Coff) !Module {
    nosuspend {
        ...
        // NOTE(Sam): we don't care about PDB at the moment
