...
void LoadRandomVariation(Engine& engine, f32 amount) {
    ...
    for (auto const i : Range(k_num_layers))
        pinned_was_sampler[i] =
            engine.pinned_snapshot.state.inst_ids[i].TryGet<sample_lib::InstrumentId>() != nullptr;
    ...
    for (auto const i : Range(k_num_layers)) {
        if (engine.pinned_snapshot.state.inst_ids[i].tag == InstrumentType::WaveformSynth) continue;
        auto const p = pinned_was_sampler[i] ? remove_probability : add_probability;
        if (RandomFloatInRange<f32>(engine.random_seed, 0, 1) < p)
            will_have_sampler[i] = !will_have_sampler[i];
    }

    // Guarantee at least one sampler remains if the pinned state had any.
    bool any_pinned = false;
    bool any_will = false;
    for (auto const v : pinned_was_sampler)
        if (v) any_pinned = true;
    for (auto const v : will_have_sampler)
        if (v) any_will = true;
    if (any_pinned && !any_will) {
        DynamicArrayBounded<u32, k_num_layers> originals {};
        for (auto const i : Range<u32>(k_num_layers))
            if (pinned_was_sampler[i]) dyn::Append(originals, i);
        auto const pick = RandomIntInRange<usize>(engine.random_seed, 0, originals.size - 1);
        will_have_sampler[originals[pick]] = true;
    }
    ...
    for (auto const i : Range(k_num_layers))
        if (auto const* p = engine.pinned_snapshot.state.inst_ids[i].TryGet<sample_lib::InstrumentId>()) {
            reference_id = p;
            break;
        }

    auto const pinned_sampler_count = ({
        u32 n = 0;
        for (auto const v : pinned_was_sampler)
            if (v) ++n;
        n;
    ...
    for (auto const layer_index : Range(k_num_layers)) {
        auto const& pinned_id = engine.pinned_snapshot.state.inst_ids[layer_index];
        ...
        if (pinned_sampler && pinned_sampler_count >= 2) {
            ...
            if (RandomFloatInRange<f32>(engine.random_seed, 0, 1) < keep_probability) continue;
        }

        auto const* origin = pinned_sampler ? pinned_sampler : reference_id;
        if (!origin) continue;
        if (auto const new_id =
                PickRandomInstrument(engine.random_seed, scratch_arena, libraries, *origin, amount))
    ...
    VarySubsetOfParams(engine.random_seed,
    ...
    for (auto const& info : k_effect_info)
        if (engine.pinned_snapshot.state.param_values[ToInt(info.on_param_index)] > 0.5f)
    ...
    for (auto const layer_index : Range<u32>(k_num_layers)) {
        ...
        if (RandomFloatInRange<f32>(engine.random_seed, 0, 1) >= octave_shift_probability) continue;

        constexpr Array k_octave_choices = {-24.0f, -12.0f, 12.0f, 24.0f};
        auto const shift =
            k_octave_choices[RandomIntInRange<usize>(engine.random_seed, 0, k_octave_choices.size - 1)];
        ...
        snapshot.param_values[ToInt(param)] =
            Clamp(engine.pinned_snapshot.state.param_values[ToInt(param)] + shift,
                  desc.linear_range.min,
                  desc.linear_range.max);
    }
