...
ErrorCodeCategory const gui_platform_error_code {
    .category_id = "GUIP",
    ...
    if (platform.last_result.wants.text_input || platform.last_result.wants.keyboard_keys.AnyValuesSet()) {
        ...
        if constexpr (IS_WINDOWS) {
            if (!platform.windows_keyboard_hook_added) {
                AddWindowsKeyboardHook(platform);
...
UiSize DefaultUiSize(GuiPlatform& platform) { return DefaultUiSizeFromDpi(platform); }
...
ErrorCodeOr<void> CreateView(GuiPlatform& platform) {
    ...
    if (auto const floe_custom_host = CustomFloeHost(platform.host)) {
    ...
    TRY(Required(puglSetEventFunc(platform.view, EventHandler)));
...
void DestroyView(GuiPlatform& platform) {
    ...
    if constexpr (IS_WINDOWS) {
        if (platform.windows_keyboard_hook_added) RemoveWindowsKeyboardHook(platform);
    }

    CloseNativeFilePicker(platform);
    ...
    SetTimers(platform, SetTimerType::Stop);
    ...
    if (!CustomFloeHost(platform.host)) {
...
ErrorCodeOr<void> SetVisible(GuiPlatform& platform, bool visible, Engine& engine) {
    ...
    if (visible) {
        ...
        if (!puglGetNativeView(platform.view)) {
            ...
            platform.double_click_time_ms = DoubleClickTimeMs(platform);
            if constexpr (IS_LINUX) X11SetParent(platform.view, puglGetParent(platform.view));
        }

        // Start timers if needed.
        SetTimers(platform, SetTimerType::Start);
