...
auto constexpr k_library_inspector_command_line_args_defs =
    MakeCommandLineArgDefs<LibraryInspectorCliArgId>({
        {
            .id = (u32)LibraryInspectorCliArgId::LibraryPath,
            .key = "library",
            .description =
                "Path to the library file (floe.lua or .mdata) or to a directory containing one.",
            .value_type = "path",
            .required = true,
            .num_values = 1,
        },
        {
            ...
            .key = "format",
            .description =
                "Output format: 'json' (default) or 'lua'. Lua output is a single `return { ... }`\n"
