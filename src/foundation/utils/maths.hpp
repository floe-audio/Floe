...
PUBLIC ALWAYS_INLINE constexpr u8 Popcount(T x) {
    return (u8)__builtin_popcountg(x);
}
...
// Max relative error ~1.1e-7. Floor(x) must fit the double's exponent range.
PUBLIC_INLINE f64 Exp2Fast(f64 x) {
    auto const integer_part = Floor(x);
    ASSERT_HOT(integer_part >= -1022 && integer_part <= 1023);
    auto const f = x - integer_part; // [0, 1)

    // 2^f via the polynomial.
    f64 u = 0.0018964611454333148;
    u = u * f + 0.0089428289841091291;
    u = u * f + 0.055866246304520702;
    u = u * f + 0.24013971109076948;
    u = u * f + 0.69315475247516734;
    u = u * f + 0.99999989311082671;

    // 2^integer_part, built directly in a double's exponent bits.
    constexpr s64 k_f64_exponent_bias = 1023;
    constexpr s64 k_f64_mantissa_bits = 52;
    auto const scale =
        __builtin_bit_cast(f64, (u64)(((s64)integer_part + k_f64_exponent_bias) << k_f64_mantissa_bits));
    return u * scale;
}

// Quarter-sine fade curve: maps [0, 1] to [0, 1] following sin(x * pi/2).
// Polynomial approximation generated by lolremez:
//   lolremez --float -d 4 -r "0:1" "sin(x*pi/2)/x"
